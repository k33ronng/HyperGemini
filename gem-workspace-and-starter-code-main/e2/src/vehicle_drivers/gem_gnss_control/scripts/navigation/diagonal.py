...
class SimpleDiagonalPark:
    def __init__(self):
        # rospy.init_node("simple_diagonal_parking")
        ...
        rospy.loginfo("✅ Connected to /ackermann_cmd. Starting diagonal parking maneuver.")
    ...
    def run_parking_maneuver(self):
        ...
        self.send_cmd(steer_deg=0.0, speed=0.0, duration=5.0)

        # # Step 5: Reverse straight to center the car
        rospy.loginfo("[Step 5] Reverse straight")
        self.send_cmd(steer_deg=0.0, speed=reverse_speed, duration=4)

        # Step 6: Stop
        rospy.loginfo("[Step 6] Stop")
        self.send_cmd(steer_deg=-max_steer, speed=0.0, duration=5)

        # Step 7: Reverse with left turn
        rospy.loginfo("[Step 7] Turn left and reverse")
        self.send_cmd(steer_deg=-max_steer, speed=reverse_speed, duration=6)
