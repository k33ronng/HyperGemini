...
class LaneNetDetector:
    ...
    def generate_waypoints(self, lane_mask):
        ...
        for lb, rb in zip(left_boundary, right_boundary):
            if lb is not None and rb is not None:
                left_lane_detected +=1
                right_lane_detected +=1
                # Use both boundaries
        ...
        print("Left lane detected: ", left_lane_detected)
        print("Right lane detected: ", right_lane_detected)

        # Use only the first 7 waypoints (closest to vehicle)
        path.poses = path.poses[:10]
        if len(path.poses)>=3:
            if is_no_line is True and (path.poses[2].pose.position.x> path.poses[1].pose.position.x):
                print("No line detected, adjusting waypoints")
                for i in range(len(path.poses)):
                    path.poses[i].pose.position.x = path.poses[i].pose.position.x + 0.4 * self.estimated_lane_width_pixels
            elif is_no_line is True and (path.poses[2].pose.position.x < path.poses[1].pose.position.x):
                print("No line detected, adjusting waypoints")
                for i in range(len(path.poses)):
                    path.poses[i].pose.position.x = path.poses[i].pose.position.x - 0.4 * self.estimated_lane_width_pixels
    ...
    def publish_waypoints(self, waypoints):
        ...
        if self.endgoal is not None:
            ...
            printf("No end goal available for publishing.")
